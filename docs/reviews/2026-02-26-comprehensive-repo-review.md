...
- **File:** `ansible/roles/komodo/templates/compose.env.j2:22-114`
...
- **Issue:** Passes secrets as command-line arguments via `ansible.builtin.shell`
  to `op item edit`. All tasks use `no_log: true`, but secrets are still visible
  in `/proc` while the shell process runs. Superseded by `backends/1password.yml`
  and `backends/infisical.yml`. Dead code with zero references from any task file
...
- **Issue:** No version pin on external role. `geerlingguy.docker` is
  properly pinned to `7.8.0`, but `bpbradley.komodo` could introduce
...
#### CQ-09: ansible.cfg Inventory Path Is Imprecise (MEDIUM)

- **File:** `ansible/ansible.cfg:2`
- **Issue:** Config sets `inventory = inventory` which works (Ansible
  auto-discovers directory contents including `all.yml`) but is imprecise.
  The real issue is CLAUDE.md referencing a non-existent `hosts.yml` file.
- **Fix:** Update CLAUDE.md to reference the actual inventory file `all.yml`
  and optionally make `ansible.cfg` explicit with `inventory = inventory/all.yml`.
...
- **Issue:** `.mise.toml` specifies `python = "3.13.7"` and CLAUDE.md matches,
  so Python is not drifted. The real drift is `ansible-core 2.19.1` (CLAUDE.md)
  vs `2.19.2` (lock file). Additionally, CLAUDE.md still references `pre-commit`
  which was migrated to `prek`.
- **Fix:** Update CLAUDE.md to reflect `ansible-core` lock file version and
  replace `pre-commit` references with `prek`. Reference `.mise.toml` as source
  of truth instead of listing versions.
...
- **Files:** `.pre-commit-config.yaml` (`v1.35.1`), `.mise.toml` (`1.37.1`)
- **Issue:** `.mise.toml` specifies `1.37.1` but `.pre-commit-config.yaml` uses
  `v1.35.1`. These are the only two sources that differ.
- **Action:** Align versions across configuration files.
- **Risk:** LOW
